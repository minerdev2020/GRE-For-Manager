...
class InfoFragment2 : Fragment(), OnSaveDataListener {
    ...
    private var binding: FragmentInfo2Binding? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        // 준공년월 초기화
        binding.houseModify2ImageButtonBuiltDate.setOnClickListener { v: View? -> showDatePickerDialog(OnClickListener { v1: View?, result: String? -> binding.houseModify2TextViewBuiltDate.text = result }) }


        // 입주가능일 초기화
        binding.houseModify2ImageButtonMoveDate.isEnabled = false
        binding.houseModify2ImageButtonMoveDate.setOnClickListener { v: View? -> showDatePickerDialog(OnClickListener { v1: View?, result: String? -> binding.houseModify2TextViewMoveDate.text = result }) }
    ...
    override fun checkData(): Boolean {
        ...
        return if (!binding!!.houseModify2CheckBoxMoveNow.isChecked
    ...
    fun initData() {
        ...
        houseType = Constants.Companion.getInstance().HOUSE_TYPE.get(house!!.house_type.toInt())
    ...
    private fun loadData() {
        ...
        binding!!.houseModify2EditTextAreaPyeong.setText(if (house!!.area_meter == 0f) "" else String.format("%.2f", house!!.area_meter * Constants.Companion.getInstance().METER_TO_PYEONG))
        binding!!.houseModify2EditTextRentAreaPyeong.setText(if (house!!.rent_area_meter == 0f) "" else String.format("%.2f", house!!.rent_area_meter * Constants.Companion.getInstance().METER_TO_PYEONG))
    ...
    private fun setSpinners() {
        // 구조 초기화
        val arrayAdapterStructure = ArrayAdapter<String>(context!!, android.R.layout.simple_spinner_dropdown_item)
        ...
        // 욕실 갯수 초기화
        val arrayAdapterBathroom = ArrayAdapter<String>(context!!, android.R.layout.simple_spinner_dropdown_item)
        ...
        // 방향 초기화
        val arrayAdapterDirection = ArrayAdapter<String>(context!!, android.R.layout.simple_spinner_dropdown_item)
                ...
                if (binding!!.houseModify2EditTextAreaPyeong.hasFocus()) {
                    ...
                    if (temp == "") {
                        ...
                        binding!!.houseModify2EditTextAreaMeter.setText(String.format("%.2f",
                                area * Constants.Companion.getInstance().PYEONG_TO_METER))
                    }
                }
            }
                ...
                if (binding!!.houseModify2EditTextAreaMeter.hasFocus()) {
                    ...
                    if (temp == "") {
                        ...
                        binding!!.houseModify2EditTextAreaPyeong.setText(String.format("%.2f",
                                area * Constants.Companion.getInstance().METER_TO_PYEONG))
                    }
                }
            }
